...
public class LoginSteps {
    ...
    public void que_estou_na_tela_de_login() throws IOException {
    ...
    public void preencho_login_e_senha(String email, String senha) throws IOException {
        loginPage.preencherEmail(email);
        loginPage.preencherSenha(senha);
    }

    @Quando("clico em Login")
    public void clico_em_Login() throws IOException {
        loginPage.clicarLogin();
    }

    @Então("vejo mensagem de login com sucesso")
    public void vejo_mensagem_login_sucesso() throws IOException {

        loginPage.verificaLoginSucesso();
    }

    @Então("vejo mensagem {string} de campo não preenchido")
    public void vejo_mensagem_de_campo_nao_preenchido(String message) throws IOException {

        loginPage.verificaCampoVazio(message);
    }

    @Quando("acesso a tela de cadastro de usuário")
    public void acesso_a_tela_de_cadastro_de_usuario() throws IOException {
        loginPage.acessarTelaCadastro();
    }

    @Dado("que estou logado na aplicação com user {string} e senha {string}")
    public void que_estou_logado_na_aplicacao_com_user_e_senha(String email, String senha) throws IOException {
        loginPage.acessarAplicacao();
        loginPage.acessarTelaLogin();
        loginPage.preencherEmail(email);
        loginPage.preencherSenha(senha);
